<x-page-shell
    title="Comptabilité · Paramètres · Catégories dépenses"
    subtitle="Personnaliser les catégories de dépenses et leurs libellés." 
>
    <div class="grid gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            <h3 class="text-base font-semibold text-gray-900">Catégories</h3>
            <div class="mt-4 flex flex-wrap gap-2">
                @foreach (['Salaires', 'Entretien', 'Matériel', 'Charges'] as $category)
                    <span class="rounded-full bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-600">{{ $category }}</span>
                @endforeach
            </div>
        </div>
